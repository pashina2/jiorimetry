...
この repository が持っているものは回路ではありません。導出です。

書き写した回路は、なぜ動くかを自分で説明できません。部品が一つ変わると作り直しになり、
次の回路に持ち越せるものが残りません。ここでは逆の順で進みます。game が計算に使っている
規則を source から読み、人が読める表に書きます。その表を問いに当てて、配置済みの成果物まで
を一本の鎖で繋ぎます。表には回路の形を入れません。形を入れた表は答えを先に書いた表です。
その下流で起きることは導出ではなく、知っている物の確認です。

読む面は三つあります。規則の写しは安く、候補のすべてに掛けられます。ただし写しの正しさは、
source を読んだ人の正しさを超えません。合成世界は game の code をそのまま走らせます。ただし
誰かが遊んでいる save ではありません。実際の world は、誰も整えていない chunk と隣接物の中で
成果物が立たなければならない唯一の場所です。手前の二つの面は失敗を安くするためにあります。
安い面での読みは最後の面についての予測です。予測は最後の面の代わりになりません。

判定器は world と速さだけが違うべきです。それ以外が違えば、判定器は world ではなく自分の都合
を答えています。判定を安くする近道は、どれも計器の便法であって game の性質ではありません。
時間のない不動点。駆動せずに固定した入力。一つだけの初期状態。この repository が報告する失敗
は、規則の失敗ではなく便法の失敗です。どれも world に訊くまで見えませんでした。便法は、何も
隠していないと示せている間だけ残します。隠した瞬間に退けます。

要求の形は一つに保ちます。入力の系列と、そこから期待する出力の表です。性質が増えるたびに
新しい checker を作ると、計器が回路より速く育ちます。育った計器は費用に見合いません。

導出を言語モデルに任せると、費用と危険を実行のたびに払います。逆引きできるものは逆引きします。
順方向の規則を逆から読めば、探索に要る候補手が出ます。うまくいった分解は、再利用してよい
条件と一緒に置きます。そうすると知識はモデルから機械へ移ります。モデルに残るのは分解です。
それも、導出済みの語彙の上の有限な探索へ縮めていきます。

作業の順も同じ理屈から出ます。要求を疑う。要らないものを消す。残りを単純にする。周回を
縮める。それから自動化する。この順です。消すべきだった物を自動化しないためです。

尺度は鎖の全体です。工程は、問いから検証済みの読みまでの周回を縮めるときに作ります。訂正は、
次に導ける回路の範囲を広げるときに費用を払います。それ以外の改善は、鎖のどこかに費用を
移しただけです。
