...
2. **world**: 1 つの void world に 6 個の artifact を z 方向に 10 cell 以上離して置く（各 artifact の原点を記録）。`build_world2.py` を写し（block entity 付きで region を書く 4 行）、支持床を足す。<operator-save>、aiwb、`tools/**` には触れない。
...
## 費用 / 席
Opus 席、上限 2.5 h、目安 300k token。host で全走は同時 1 本以下（他の重い process を起こさない）。web なし、`<reference-circuit-records>/**` 禁止、world / aiwb / tools / git に触れない。
