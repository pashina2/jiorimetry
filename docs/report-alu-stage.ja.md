...
方針の出所: オペレータと Astra（第二 model の査読者）（2026-09-07 19:01Z）が定めた方針 — 既存回路の解法やオペレータの経験を設計の前提にしない、規則と実験で詰める、検証済み構成の再利用は可、未知の接続はオペレータに尋ねず小さく検証する（`docs/alu-place-handover.md` 冒頭）。
