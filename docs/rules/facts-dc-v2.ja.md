# DC 信号強度演算の事実表 v2（PLACE-ALU-1 の source 検証 2 点を反映、2026-09-07T18:01Z。元 = DERIVE-2 の与件、2026-09-07T13:18Z、DIRECTOR 7 が 1.20.6-yarn から読んで書いた。回路の形は含まない）

> English: [facts-dc-v2.md](facts-dc-v2.md)
...
- side の値 j（`getMaxInputLevelSides` :141-147）: 2 側の max。読める源 = redstone_block 15、wire の power、**gate の出力（その gate の facing が読み手へ向いている時だけ、`AbstractRedstoneGateBlock.getWeakRedstonePower` :81-88）**。**solid の受電は side からは読めない**（3 引数版に solid の節が無い）。**torch も side からは 0**（side は strong power を読み、`RedstoneTorchBlock.getStrongRedstonePower` :103-108 は DOWN 以外 0）。**container も side からは読めない**（container の値を読むのは back の `ComparatorBlock.getPower` だけ）。
...
- torch: 取り付け先の block が受電していなければ 15、していれば 0（反転）。上の block を strong power する。**comparator の back からは weak 15 として読める**（`RedstoneView.getEmittedRedstonePower` 2 引数版 :66-73 は weak を読む、torch の weak は UP 以外 15 :64-69）が、**side からは 0**（上の side の項）。
